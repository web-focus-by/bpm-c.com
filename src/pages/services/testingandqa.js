...
const Testingandqa = ({ location }) => {
  const getData = useStaticQuery(graphql`
    query siteGetTestingandqaDataQuery {
      allWpPost {
        edges {
          node {
            id
      ...
      wpPage(slug: {eq: "testingandqa"}) {
        id
        uri
        title
        content
      }
      allWpPage(filter: {wpParent: {node: {slug: {eq: "testingandqa"}}}}) {
        edges {
          node {
            id
            title
  ...
  return (
    <>
      <Layout>
        <HeroWebSiteDesign content={ contentPage } location={ location } crumbLabel="Testing and QA"></HeroWebSiteDesign>
        <ServiceITOutsourcing title={ "Testing and QA" } themes={ themes }></ServiceITOutsourcing>
