...
const Servicesitoutstaffing = ({ location }) => {
  const getData = useStaticQuery(graphql`
    query siteGetServicesitoutstaffingDataQuery {
      allWpPost {
        edges {
          node {
            id
      ...
      wpPage(slug: {eq: "servicesitoutstaffing"}) {
        id
        uri
        title
        content
      }
      allWpPage(filter: {wpParent: {node: {slug: {eq: "servicesitoutstaffing"}}}}) {
        edges {
          node {
            id
            title
  ...
  return (
    <>
      <Layout>
        <HeroWebSiteDesign content={ contentPage } location={ location } crumbLabel="IT Outstaffing"></HeroWebSiteDesign>
...
export default Servicesitoutstaffing