...
const Uiuxmobileappdesign = ({ location }) => {
  const getData = useStaticQuery(graphql`
    query siteGetUiuxmobileappdesignDataQuery {
      ...
      wpPage(slug: {eq: "uiuxmobileappdesign"}) {
        id
        uri
        title
        content
      }
      allWpPage(filter: {wpParent: {node: {slug: {eq: "uiuxmobileappdesign"}}}}) {
        edges {
          node {
            id
            title
  ...
  return (
    <>
      <Layout>
        <HeroWebSiteDesign content={ contentPage } location={ location } crumbLabel="UI/UX mobile application design"></HeroWebSiteDesign>
