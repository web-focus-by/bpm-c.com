...
const Gamedev = ({ location }) => {
  const getData = useStaticQuery(graphql`
    query siteGetGamedevDataQuery {
      allWpPost {
        edges {
          node {
            id
      ...
      wpPage(slug: {eq: "gamedev"}) {
        id
        uri
        title
        content
      }
      allWpPage(filter: {wpParent: {node: {slug: {eq: "gamedev"}}}}) {
        edges {
          node {
            id
            title
  ...
  return (
    <>
      <Layout>
        <HeroWebSiteDesign content={ contentPage } location={ location } crumbLabel="Game Development"></HeroWebSiteDesign>
        <ServiceITOutsourcing title={ "Game Development" } themes={ themes }></ServiceITOutsourcing>
