...
const Landingpagedevelopment = ({ location }) => {
  const getData = useStaticQuery(graphql`
    query siteGetLandingpagedevelopmentDataQuery {
      allWpPost {
        edges {
          node {
            id
      ...
      wpPage(slug: {eq: "landingpagedevelopment"}) {
        id
        uri
        title
        content
      }
      allWpPage(filter: {wpParent: {node: {slug: {eq: "landingpagedevelopment"}}}}) {
        edges {
          node {
            id
            title
  ...
  return (
    <>
      <Layout>
        <HeroWebSiteDesign content={ contentPage } location={ location } crumbLabel="Landing page development"></HeroWebSiteDesign>
        <ServiceITOutsourcing title={ "Landing page development" } themes={ themes }></ServiceITOutsourcing>
