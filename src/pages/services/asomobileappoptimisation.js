...
const Asomobileappoptimisation = ({ location }) => {
  const getData = useStaticQuery(graphql`
    query siteGetAsomobileappoptimisationDataQuery {
      allWpPost {
        edges {
          node {
            id
      ...
      wpPage(slug: {eq: "asomobileappoptimisation"}) {
        id
        uri
        title
        content
      }
      allWpPage(filter: {wpParent: {node: {slug: {eq: "asomobileappoptimisation"}}}}) {
        edges {
          node {
            id
            title
  ...
  return (
    <>
      <Layout>
        <HeroWebSiteDesign content={ contentPage } location={ location } crumbLabel="ASO mobile app optimisation"></HeroWebSiteDesign>
        <ServiceITOutsourcing title={ "ASO mobile app optimisation" } themes={ themes }></ServiceITOutsourcing>
...
export default Asomobileappoptimisation