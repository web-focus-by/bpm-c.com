...
const Webdevelopmentservices = ({ location }) => {
  const getData = useStaticQuery(graphql`
    query siteGetWebdevelopmentservicesDataQuery {
      allWpPost {
        edges {
          node {
            id
      ...
      wpPage(slug: {eq: "webdevelopmentservices"}) {
        id
        uri
        title
        content
      }
      allWpPage(filter: {wpParent: {node: {slug: {eq: "webdevelopmentservices"}}}}) {
        edges {
          node {
            id
            title
  ...
  return (
    <>
      <Layout>
        <HeroWebSiteDesign content={ contentPage } location={ location } crumbLabel="Web development services"></HeroWebSiteDesign>
        <ServiceITOutsourcing title={ "Web development services" } themes={ themes }></ServiceITOutsourcing>
