...
const Smm = ({ location }) => {
  const getData = useStaticQuery(graphql`
    query siteGetSmmDataQuery {
      ...
      allWpPage(filter: {wpParent: {node: {slug: {eq: "smm"}}}}) {
        edges {
          node {
            id
            title
  ...
  return (
    <>
      <Layout>
        <HeroWebSiteDesign content={ contentPage } location={ location } crumbLabel="SMM"></HeroWebSiteDesign>
        <ServiceITOutsourcing title={ "SMM" } themes={ themes }></ServiceITOutsourcing>
