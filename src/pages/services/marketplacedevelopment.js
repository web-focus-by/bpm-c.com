...
const Marketplacedevelopment = ({ location }) => {
  const getData = useStaticQuery(graphql`
    query siteGetMarketplacedevelopmentDataQuery {
      ...
      wpPage(slug: {eq: "marketplacedevelopment"}) {
        id
        uri
        title
        content
      }
      allWpPage(filter: {wpParent: {node: {slug: {eq: "marketplacedevelopment"}}}}) {
        edges {
          node {
            id
            title
  ...
  return (
    <>
      <Layout>
        <HeroWebSiteDesign content={ contentPage } location={ location } crumbLabel="Marketplace development"></HeroWebSiteDesign>
