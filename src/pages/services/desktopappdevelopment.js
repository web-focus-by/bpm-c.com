...
const Desktopappdevelopment = ({ location }) => {
  const getData = useStaticQuery(graphql`
    query siteGetDesktopappdevelopmentDataQuery {
      allWpPost {
        edges {
          node {
            id
      ...
      wpPage(slug: {eq: "desktopappdevelopment"}) {
        id
        uri
        title
        content
      }
      allWpPage(filter: {wpParent: {node: {slug: {eq: "desktopappdevelopment"}}}}) {
        edges {
          node {
            id
            title
  ...
  return (
    <>
      <Layout>
        <HeroWebSiteDesign content={ contentPage } location={ location } crumbLabel="Desktop application development"></HeroWebSiteDesign>
        <ServiceITOutsourcing title={ "Desktop application development" } themes={ themes }></ServiceITOutsourcing>
