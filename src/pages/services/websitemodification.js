...
const Websitemodification = ({ location }) => {
  const getData = useStaticQuery(graphql`
    query siteGetWebsitemodificationDataQuery {
      allWpPost {
        edges {
          node {
            id
      ...
      wpPage(slug: {eq: "websitemodification"}) {
        id
        uri
        title
        content
      }
      allWpPage(filter: {wpParent: {node: {slug: {eq: "websitemodification"}}}}) {
        edges {
          node {
            id
            title
  ...
  return (
    <>
      <Layout>
        <HeroWebSiteDesign content={ contentPage } location={ location } crumbLabel="Website modification"></HeroWebSiteDesign>
        <ServiceITOutsourcing title={ "Website modification" } themes={ themes }></ServiceITOutsourcing>
...
export default Websitemodification